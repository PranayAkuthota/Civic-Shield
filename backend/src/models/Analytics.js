...
const categoryStatSchema = new _mongoose.Schema({
  name: {
    type: String,
    required: true,
...
const hotspotSchema = new _mongoose.Schema({
  coordinates: {
    type: [Number],
    required: true,
    validate: {
...
const analyticsSchema = new _mongoose.Schema({
  date: {
    type: Date,
...
analyticsSchema.virtual('dominantCategory').get(function () {
  ...
  return this.categories.reduce((prev, current) => prev.count > current.count ? prev : current);
});
...
analyticsSchema.statics.getByDateRange = function (startDate, endDate, district) {
  const query = {
    date: {
      $gte: startDate,
      $lte: endDate
...
analyticsSchema.statics.getStateAnalytics = function (startDate, endDate) {
  return this.find({
    date: {
      $gte: startDate,
      $lte: endDate
...
analyticsSchema.statics.generateDailyAnalytics = function (date = new Date()) {
  const Complaint = _mongoose.default.model('Complaint');
...
analyticsSchema.statics.getTrendingCategories = function (district) {
  const Complaint = _mongoose.default.model('Complaint');
...
// Get monthly trends
analyticsSchema.statics.getMonthlyTrends = function (months = 12, district) {
  const matchStage = {};
  if (district && district !== 'ALL') {
...
analyticsSchema.statics.generateAndSave = async function (date = new Date()) {
  try {
...
// Export the model
const Analytics = exports.Analytics = _mongoose.default.model('Analytics', analyticsSchema);
