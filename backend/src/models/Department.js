...
const departmentSchema = new _mongoose.Schema({
  ...
  jurisdiction: {
    ...
    enum: ['state',
    // State-wide department
    'district',
    // District-level department
    'regional',
    // Regional department covering multiple districts
    'municipal',
...
departmentSchema.methods.addOfficial = function (officialId) {
  if (!this.officials) {
    this.officials = [];
  }
...
departmentSchema.methods.removeOfficial = function (officialId) {
  if (this.officials) {
...
departmentSchema.methods.hasAccess = function (userId) {
...
departmentSchema.statics.findByJurisdiction = function (jurisdiction) {
  return this.find({
...
departmentSchema.statics.findByUserAccess = function (userId) {
  return this.find({
    $and: [{
      isActive: true
    }, {
...
departmentSchema.statics.departmentExists = async function (departmentId) {
...
// Export the model
const Department = exports.Department = _mongoose.default.model('Department', departmentSchema);
