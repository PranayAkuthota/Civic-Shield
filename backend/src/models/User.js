...
userSchema.pre('save', async function (next) {
  ...
  try {
    ...
    console.log("HASHING PASSWORD:", typeof this.passwordHash, this.passwordHash); this.passwordHash = await _bcryptjs.default.hash(this.passwordHash, saltRounds);
    next();
  } catch (error) {
    next(error);
  }
...
userSchema.methods.comparePassword = async function (candidatePassword) {
  ...
  return _bcryptjs.default.compare(candidatePassword, this.passwordHash);
};
...
userSchema.methods.generateEmailVerificationToken = function () {
  const verifyToken = _crypto.default.randomBytes(32).toString('hex');
  return verifyToken;
};
...
userSchema.statics.findOfficialsByDepartment = function (department) {
  return this.find({
    role: 'official',
...
userSchema.statics.aadhaarExists = async function (aadhaar) {
...
userSchema.virtual('profile').get(function () {
  return {
    ...
    maskedAadhaar: this.aadhaar ? `XXXXXXXX${this.aadhaar.replace(/\s/g, '').slice(-4)}` : '',
