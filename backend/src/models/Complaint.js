...
exports.default = exports.Complaint = void 0;
...
const locationSchema = new _mongoose.Schema({
  ...
  coordinates: {
    ...
    validate: {
      validator: function (coords) {
        return coords.length === 2 && coords[0] >= 74 && coords[0] <= 80 &&
        // Longitude bounds for Telangana
  ...
  district: {
    ...
    enum: ['Adilabad', 'Bhadradri Kothagudem', 'Hanumakonda', 'Hyderabad', 'Jagtial', 'Jangaon', 'Jayashankar Bhupalpally', 'Jogulamba Gadwal', 'Kamareddy', 'Karimnagar', 'Khammam', 'Kumuram Bheem', 'Mahabubabad', 'Mahabubnagar', 'Mancherial', 'Medak', 'Medchal Malkajgiri', 'Mulugu', 'Nagarkurnool', 'Nalgonda', 'Narayanpet', 'Nirmal', 'Nizamabad', 'Peddapalli', 'Rajanna Sircilla', 'Rangareddy', 'Sangareddy', 'Siddipet', 'Suryapet', 'Vikarabad', 'Wanaparthy', 'Warangal', 'Yadadri Bhuvanagiri']
  },
  mandal: {
    type: String,
...
const evidenceFileSchema = new _mongoose.Schema({
  filename: {
    type: String,
...
const statusHistorySchema = new _mongoose.Schema({
  status: {
    type: String,
...
const feedbackSchema = new _mongoose.Schema({
  userId: {
    type: _mongoose.Schema.Types.ObjectId,
    ref: 'User',
...
const complaintSchema = new _mongoose.Schema({
  complaintId: {
    type: String,
    required: true,
    unique: true,
...
complaintSchema.methods.addStatusHistory = function (status, updatedBy, comment) {
...
complaintSchema.methods.assignTo = function (officialId, assignedBy) {
...
// Get average rating
complaintSchema.methods.getAverageRating = function () {
...
complaintSchema.methods.canBeViewedBy = function (userId, userRole) {
  ...
  if (this.filedBy && (this.filedBy._id || this.filedBy).toString() === userId) return true;

  // Assigned officials
  if (this.assignedTo && (this.assignedTo._id || this.assignedTo).toString() === userId) return true;

  // Current handler
  if (this.currentHandler && (this.currentHandler._id || this.currentHandler).toString() === userId) return true;

  // Officials, Admins and superadmins
  if (userRole === 'official' || userRole === 'admin' || userRole === 'superadmin') return true;
...
complaintSchema.statics.findByUser = function (userId, includeAnonymous = true) {
  const query = {
    filedBy: userId
  };
  if (!includeAnonymous) {
...
complaintSchema.statics.findByDistrict = function (district) {
  return this.find({
...
complaintSchema.statics.findWithinRadius = function (longitude, latitude, radiusKm) {
  return this.find({
...
complaintSchema.statics.getCategoryStats = function (district) {
  const matchStage = {};
  if (district) {
...
// Export the model
const Complaint = exports.Complaint = _mongoose.default.model('Complaint', complaintSchema);
var _default = exports.default = Complaint;